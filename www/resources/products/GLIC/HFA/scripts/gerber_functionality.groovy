...
import com.stepsoln.core.db.cases.RequirementTarget
...
def String applicantTobaccoUsage;
def RequirementTarget signatureTarget;
def CaseRequirement signatureRequirement;
def SecurityUser nbOwner;
def SecurityOu nbOu;
def Long carrierId;
def Long productId;
def ConfigResource emailTemplate;
def Agent agent;
def String invitationUrl;
def Map<String, Map<String, Object>> eformsAnswers;
...
def String getProductVersionCode()
{
	...
	if (quotes.size() > 0)
	{
		ApplicationQuote quote = currentCase.getFinalQuote();
...
def getMarketingKeyInfo()
{
	...
	def defaultMarketingKeys = [internal_hfa_term_0 : '007854', internal_hfa_whole_0 : '007856',
				internal_hfa_term_1 : '402876', internal_hfa_whole_1 : '402877',
				external_hfa_term_0 : '406431', external_hfa_whole_0 : '406432']
	Case theCaseData = currentCase;
	int paperFlag = 0;
	if(Boolean.TRUE.equals(theCaseData.getIsPaperCase()))
	{
		paperFlag = 1;
	}
	String source = theCaseData.getCaseSource() == CASE_SOURCE.CAPTIVE  ? 'internal' : 'external';

	[planName : planName, defaultMarketingKey:(defaultMarketingKeys[source + '_' + planName + '_' + paperFlag] ?: null)]
}

def List<String> getExtensionInfoPrimaryInsured()
{
	def result = [];
	...
	if (m7)
	{
		...
		xml.FutureTravelInd(tc:tcValue, value);
	...
	if (m7completer && m7 && m7 == "Y")
	{
		...
		int i = 0
		while (eformsAnswers.REFLEXIVE['m7completer1___' + i])
		{
			completer += ", " + eformsAnswers.REFLEXIVE['m7completer1___' + i]
			i++
		}
		xml1 = new MarkupBuilder(writer);
		xml1.AdditionalInformation(completer);
		result.add(writer.toString());
	}
	return result;
}

def List<String> getTreatmentDescriptionPrimaryInsured()
{
	def result = [];
...
def String getUWName()
{
	...
	else if (caseStatus.equals(UW_CASE_STATUS.INCOMPLETE) && CASE_STATUS.AUTODECLINE.equals(currentCase.getStatus()))
	{
		uwName = "AUTODECL";
	}
	return uwName;
...
def String getHOAppFormNumber()
{
	...
	Date cuttOffDate=DateUtil.parseDate(DateUtil.FORMAT_SLASH_DATE, cutOffProperty);
		if(currentCase.getCreatedDate().before(cuttOffDate)){
		boolean isNonCompactState= ["AR","DC","CA","CT","DE","FL","MT","NY","ND","SD"].contains(contractLocale);
		if( isNonCompactState ){
			if( ["AR","DE"].contains(contractLocale) ){
				retVal="AWLTL-12";
			}else if (["CA","CT","NY"].contains(contractLocale)){
		...
		boolean isNonCompactState= ["DC","CA","CT","DE","FL","MT","NY","ND","SD"].contains(contractLocale);
		if( isNonCompactState ){
			if( ["DE","ND"].contains(contractLocale) ){
				retVal="AWLTL-13"+"(R)";
			}else if (["CA","CT"].contains(contractLocale)){
				retVal="AWLTL-13-"+contractLocale;
			}else if (["DC","MT","FL"].contains(contractLocale)){
			...
			}else if (["NY"].contains(contractLocale)){
				retVal="AWLTL-13-"+contractLocale+ "-R";
			}else if(["SD"].contains(contractLocale)){
				retVal="AWLTL-13"+"(R)"
			}
...
def Boolean addAmendmentInformation() {
	...
	boolean hasPolicyOwner = 'Y' == EFORMS.PERSON['oth_owner_ind'] 
	if ("NY" == currentCase.getContractLocale() &&
		(answeredY || hasPolicyOwner
	...
	{
		EFormAnswers curAnswers = (EFormAnswers)EFORMS.REFLEXIVE;
		def pdfElements = processReflexiveQuestions(QUESTIONS.REFLEXIVE,  curAnswers)
	...
	followupMap.sort().each { k, v ->
		if (k.indexOf(questionId+'_followup_question_') >= 0)
		{
			...
			if(a)
			{
				...
				else if (a instanceof Object[] || a instanceof List)
				{
					def l = []
					a.each{ entry ->
...
def Element createSectionIOwnership()
{
	...
	labelValue(p, 30, '\nName of Owner:  Last ', (String)EFORMS.OWNER_AUTH['owner_last_name'])
	labelValue(p, 30, '  First ', EFORMS.OWNER_AUTH['owner_first_name'])
	labelValue(p, 10, '  M.I. ', EFORMS.OWNER_AUTH['owner_middle_name'] ? EFORMS.OWNER_AUTH['owner_middle_name'].charAt(0):'')
...
def labelValue(Paragraph p, Integer length, String label, Object value)
{
	...
	c = new Chunk(inputField(length, valueStr), PdfUtil.HELVETICA_10_UNDERLINE)
	p.add(c)
}
...
def EmailRequestType generateEmailContentFor(Applicant app)
{
	...
	try{
		...
		CASE_REQUIREMENT_TARGET curTarget = CASE_REQUIREMENT_TARGET.PRIMARY_INSURED;
		if(app.getApplicantType() == APPLICANT_TYPE.POLICY_OWNER)
		{
			curTarget = CASE_REQUIREMENT_TARGET.POLICY_OWNER;
		}
		CaseRequirement curRequirement = RequirementsService.findCaseRequirementByTargetAndCode(currentCase.getRequirements(), curTarget, REQUIREMENT_TYPE.ESIGNATURE, "HIP");
		if(curRequirement != null)
		{
			Agent agent = getAgentFromCase();
			...
			if( resource != null && app != null )
			{
				...
				if (!StringUtils.isEmpty(app.getEmail()))
				{
					emailRequest = new EmailRequestType();
					SecurityUser curOwner = currentCase.getNbOwner();
					SecurityOu curOu = currentCase.getNbOu();
					String senderName = StringUtils.isEmpty(curOu.getSignatureLine()) ? curOwner.getSignatureLine() : curOu.getSignatureLine();
					String recipientName = RequirementsService.getSignatureRequirementTargetFullName(app);
					Carrier curCarrier = coreServices.getEntityCacheService().getCachedData(Carrier.class, currentCase.getCarrierId());
					String carrierName = curCarrier.getName();
					String productName = coreServices.getEntityCacheService().getCachedData(Product.class, currentCase.getProductId()).getName();
					if (!StringUtils.isEmpty(productName))
					{
						productName = ("euioaEUIOA".contains(productName.trim().substring(0, 1)) ? "an " : "a ") + productName;
					}
					String emailContent = TemplateHelper.velocityTranslateString(new String(resource.getContent()),
						new QuickMap("senderName", senderName)
							.add("recipientName", WordUtils.capitalize(StringUtils.lowerCase(recipientName)))
							.add("productDescription",productName)
							.add("carrierName",carrierName)
							.add("link",invitation.getInvitationUrl())
							);
					emailRequest.setAddressFrom(agent?.email);
					emailRequest.setAddressTo(app.getEmail());
					emailRequest.setBodyContent(emailContent);
					emailRequest.setSubject("Complete your " + carrierName + " Application Form");
					emailRequest.setRequirementId(curRequirement.getCaseRequirementId());
				}
...
def EmailRequestType generateEmailRequestType(RequirementTarget target, CaseRequirement curRequirement)
{
	EmailRequestType emailRequest = null;
	try{
		if(emailTemplate != null && target != null)
		{
			if (!StringUtils.isEmpty(target.getEmail()))
			{
				emailRequest = new EmailRequestType();
				
				Carrier curCarrier = coreServices.getEntityCacheService().getCachedData(Carrier.class, carrierId);
				String carrierName = curCarrier.getName();
				String productName = coreServices.getEntityCacheService().getCachedData(Product.class, productId).getName();
				if (!StringUtils.isEmpty(productName))
				{
					productName = ("euioaEUIOA".contains(productName.trim().substring(0, 1)) ? "an " : "a ") + productName;
				}
				
				String senderName = carrierName;
				if (nbOu!=null && !StringUtils.isEmpty(nbOu.getSignatureLine()))
				{
					senderName = nbOu.getSignatureLine();
				}
				else if (nbOwner!=null)
				{
					senderName = nbOwner.getSignatureLine()
				}
				
				String recipientName = RequirementsService.getSignatureRequirementTargetFullName(target);
				
				String emailContent = TemplateHelper.velocityTranslateString(new String(emailTemplate.getContent()),
					new QuickMap("senderName", senderName)
						.add("recipientName", WordUtils.capitalize(StringUtils.lowerCase(recipientName)))
						.add("productDescription",productName)
						.add("carrierName",carrierName)
						.add("link",invitationUrl)
						);
				emailRequest.setAddressFrom(agent?.email);
				emailRequest.setAddressTo(target.getEmail());
				emailRequest.setBodyContent(emailContent);
				emailRequest.setSubject("Complete your " + carrierName + " Application Form");
				emailRequest.setRequirementId(curRequirement.getCaseRequirementId());
			}
...
def Object generateEmailRequestTypeForSignatureTarget()
{
	EmailRequestType emailRequest = null;
	if (signatureTarget!=null)
	{
		...
		{
			emailRequest = generateEmailRequestType(signatureTarget, signatureRequirement);
		}
		else
		{
			emailRequest = generateEmailContentFor(signatureTarget);
		}
	}
	else
	{
		String msg = "Signature Target Party not found"
		logger.error(msg, e)
	}
	return emailRequest;
}
...
def List generateEmailContentForSendAgentForUpdate()
{
	List retList = new ArrayList();
	Agent agent = getAgentFromCase();
	...
	{
		EmailRequestType emailRequest = null;
		ConfigResource resource = coreServices.getConfigurationRepository().loadLastestResource(currentCase.getCarrierId(), currentCase.getProductId(), "glic_applicant_request_change_for_case");
		if(resource != null && app != null && !StringUtils.isEmpty(agent.getEmail()))
		{
			String recipientName = RequirementsService.getSignatureRequirementTargetFullName(agent);
			String emailContent = TemplateHelper.velocityTranslateString(new String(resource.getContent()),
				new QuickMap("recipientName", recipientName)
					.add("policyNo",currentCase.getPolicyNo())
					);
			emailRequest = new EmailRequestType();
			emailRequest.setAddressFrom(app.getEmail());
			emailRequest.setAddressTo(agent.getEmail());
			emailRequest.setBodyContent(emailContent);
			emailRequest.setSubject("Application Revision Requested");
			
			if(emailContent != null)
			{
				retList.add(emailRequest);
			}
		}
	}
...
def Agent getAgentFromCase()
{
	Agent agent = null
	Long agentId = currentCase.getAgentId();
	if (agentId != null)
	{
		agent = coreServices.getCaseService().getHibernateTemplate().get(Agent.class, agentId);
	}
	return agent;
}
...
{
	if(currentCase.getIsPaperCase() && !currentCase.isOutOfNb() )
	{
		return false;
	}
	else
...
{
	if (planCode == optionCode)
	{
		return planCode
	}
	def termAndRiders = [TERM30:[DWP:'TERM30DWP'], TERM20:[DWP:'TERMDWP'],
		TERM15:[DWP:'TERMDWP'], TERM10:[DWP:'TERMDWP'], WL:[DWP:'WLDWP', GIR:'GIR']]
	
	def riders = termAndRiders[planCode]
	def code = riders[optionCode]

	return code
}

def processCustomReflexive()
{
	String eform = EFORM_CODE
	EFormTransferHelper transHelper = TRANSFER
	NBToCaseAdapter adapter = ADAPTER
	Map<String, List<String>> incompleteEforms = INCOMPLETE
	
	int idx = 0
	String qindex = String.format("___%s", String.valueOf(idx))
	String questionInProgress = null
	try
	{
		if (eform == 'REFLEXIVE')
		{
			questionInProgress = "FamilyIllness"
			for (FamilyIllnessType illness : adapter.getPrimaryApplicant().getRisk().getFamilyIllness())
			{
				if (illness.getApplicability() == null)
				{
					throw new com.stepsoln.core.util.ApplicationException("FamilyIllness does not have Applicability information")
				}
				if (BigInteger.ONE.equals(illness.getApplicability().getTc()))
				{
					OLILUREL rrc = illness.getRelationRoleCode()
					String relation = "Unknown"
					if (rrc != null && rrc.getTc().equals(new BigInteger("3")))
					{
						relation = "Natural Parent"
					}
					else if (rrc != null && rrc.getTc().equals(new BigInteger("4")))
					{
						relation = "Sibling"
					}
		
					transHelper.setEformAnswerValue(eform, "family_disorder" + qindex, StringUtils.defaultIfEmpty(illness.getDiagnosisDesc(), ""));
					transHelper.setEformAnswerValue(eform, "family_relation" + qindex, relation);
					String age = illness.getAgeAtDeath() != null ? illness.getAgeAtDeath().toString() : ""
					String status = "Deceased"
					if (age.isEmpty())
					{
						age = illness.getOnsetAge() != null ? illness.getOnsetAge().toString() : ""
						status = "Living"
					}
					if (!age.isEmpty())
					{
						transHelper.setEformAnswerValue(eform, "family_age_of_death" + qindex, age)
						transHelper.setEformAnswerValue(eform, "family_status" + qindex, status)
					}
					idx++
					qindex = String.format("___%s", idx)
				}
			}
		}
	}
	...
	{
		List<String> eformquestionids = incompleteEforms.get(eform);
		if (eformquestionids == null)
		{
			eformquestionids = new ArrayList<String>();
		}
		eformquestionids.add(questionInProgress);
		incompleteEforms.put(eform, eformquestionids);
		
		String msg = String.format("Custom reflexive parser failed to process %s", questionInProgress)
		logger.error(msg, e)
	}
}
...
def void fillExtensionInfo()
{
	for (TXLifeRequestType txLifeRequestType: txLifeType.getTXLifeRequest())
	{
		...
		def result = getExtensionInfoPrimaryInsured();
