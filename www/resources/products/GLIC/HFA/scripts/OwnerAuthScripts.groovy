...
def importOtherOwner() {
	logger.debug("Start importing Other Owner for case id="+currentCase.getCaseId()+" hash="+currentCase.hashCode());
		...
		if (owner==null) {
			...
			owner.setUwType(APPLICANT_UW_TYPE.NOT_APPLICABLE);
		...
		address.ensureRequiredFieldsNotNull();
		List<Address> addresses = new ArrayList<Address>();
		addresses.add(address);
		owner.setAddresses(addresses);
		owner.setPrimaryAddress(address);
		owner = services.coreServices.caseService.applicantRepository.saveApplicantWithAddress(owner);
...
def prefillOtherOwner() {
	...
	if (owner!=null)
	{
		...
		{
			eform.put("owner_residence_address",owner.getPrimaryAddress().getAddressLine1());
